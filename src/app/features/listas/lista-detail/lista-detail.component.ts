...
export class ListaDetailComponent implements OnInit {
  ...
  onEditar(isPublica: boolean): void {
      const currentList = this.lista();
      if (currentList && currentList.id) {
          this.listaService.editarLista(currentList.id, {...currentList, publica: isPublica}).subscribe({
            next: () => {
